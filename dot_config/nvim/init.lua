...
NOTE: 改修計画
- LSPを明示的に有効にしない限り、無効にする
- restartコマンドでセッションが復元されるようにしたい
- ディレクトリごとにセッションを保持したい
- nvimの中にいる かつ skk-fcitx5が有効 → 無効にする
...
require('lazy').setup({
  ...
  },
  {
    ...
    config = function()
      -- ctrl-w oを入れ換える
      -- Goyoを有効にしている時はcolorful-winsep.nvimをオフにする
